...
* /var/craydentdeploy/config/ - stores config files for Craydent applications.
* /var/craydentdeploy/git/ - stores the original/current git repo files.
* /var/craydentdeploy/nodejs/ - stores all the nodejs file for each git repo.
* /var/craydentdeploy/log/ - stores the logs for each project.
* /var/craydentdeploy/backup/ - stores back up copies for each project.
* /var/craydentdeploy/key/ - stores ssh keys created and used by Craydent-deploy.

### CLI

Usage with arguments (these are defaults or variables) to initialize
```shell
$ sudo cdeploy 'prod' 4900 4800 admin admin /var/craydentdeploy/key/master_id_rsa.pub '' '{{git@github.com:craydent/Craydent-Deploy.git}}' '{{Craydent-Deploy}}' 'yes' 'yes' '{{http://www.example.com}}' '{{gituser}}' '{{gitpassword}}'
```
...
```
cdeploy add can take 7 arguments.  When arguments are missing, the CLI will ask a series of questions to obtain the missing arguments.
 ...
 * [GitHub](https://github.com/craydent/Craydent-Deploy/)
 * [BitBucket](https://bitbucket.org/craydent/craydent-deploy)
